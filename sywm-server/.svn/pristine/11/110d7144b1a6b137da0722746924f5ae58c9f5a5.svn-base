...
public class StorageAreaServerImpl implements StorageAreasService {
	...
	public List<OptionKV> queryKV() throws Exception {
		...
		String hql = "from StorageAreas where isEnable = 1 ";
		List<StorageAreas> storageAreasList = storageAreaDAO.find(hql);

		for (StorageAreas storageAreas : storageAreasList) {

			OptionKV option = new OptionKV();
			option.setText(storageAreas.getAreasName());
			option.setValue(storageAreas.getStorageAreasId());
	...
	public List<StorageAreas> selectAllAreas(Map<String, String> params) throws Exception {
		...
		String hql = "from StorageAreas where isEnable = 1 ";
		return storageAreaDAO.find(hql);
	}
